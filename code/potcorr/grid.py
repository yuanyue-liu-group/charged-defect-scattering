"""Real-space grid utilities that bridge the small and large supercells.

`rho_expand` (zero-pad) + `downsample_3d_array` (coarsen) implement the SM's
"FFT -> real space -> zero the outer region -> FFT" step that carries
rho_bare from the 6x6 defect cell onto the 9x9 / 24x24 rescreening grid.
"""

import numpy as np

from .const import Bohr_R
